...
![*Figure 1. (Left) Distribution of adoption speeds of animals in the dataset separated by Cats vs Dogs where 0 = Adopted same day as posting, 1 = Adopted 1-7 days since posting, 2 = Adopted 8-30 days since posting, 3 = Adopted 31 - 90 days since posting, 4 = Adopted 91 + days since posting. (Right) The quantity of animals included in the posting.*]( /home/rstudio/work/results/figures/cat_dog_adopt_rate_quantity_per_post.png){ width=500px }
...
![*Figure 2. Overview of various features of the data set including, the gender of the animal, the size at maturity of the animal, the hair length, vaccination status, dewormed status, whether the animal has been spayed/neutered, the health status of the animal as it was put up for adoption, and the main fur color.*](/home/rstudio/work/results/figures/single_animals_overview_plots.png){ width=400px }
...
![*Figure 3. Overview of various features of the data set including, the gender of the animal, the size at maturity of the animal, the hair length, vaccination status, dewormed status, whether the animal has been spayed/neutered, the health status of the animal as it was put up for adoption, and the main fur color.* ](/home/rstudio/work/results/figures/adoption_fee_distribution.png){ width=300px }
...
![*Figure 5. (Left) Most common dog and cat names for animals with a name. 'Unknown' was the most common, but this was removed such that we show animals assigned names. (Right) The age distribution of the anmials in years.*](/home/rstudio/work/results/figures/single_animals_top_cat_dog_names_ages.png){width=700px}
...
After comparing models, we can see that the Ridge Classifier has the highest Accuracy, Gradient Boosting Classifier has the highest Precision and F1 score, and the ExtraTrees Classifier ranks third in line, and finally the Random Forest Classifier has the highest Area Under the Curve (AUC).  These models, however, all have very low accuracy, ranging from 0.38 to 0.40.

Before we continue, let's generate our Extra Trees Classifier model and look at the important features. 
![*Figure 6. Top 10 important features in extra tree classifier model](/home/rstudio/work/results/figures/et_model_top_features_importance_nofeature_selection_plot.png'){width=700px}

If we look at our total features we see we have 65, however, if we set a threshold and only keep features with an importance > 0.1, we see we hae 25 important features. Subsetting our training data to only contain these features and running the compare models function, we see a slight improvement, where our top four classifiers are Gradient Boosting, Random Forest, Ada Boost, and Decision Tree, with accuracy values ranging from 0.52 to 0.55.

Great! Let's use our updated set of features to tune our top four models and then predict on our unseen data. 

When we move to create and tune our models, we see that the original model is better than our tuned model for the top three models. This indicates that our models are likely overfit, and perhaps we chose too few features. But let's visualize a few results of our top 3 models without tuning anyways.

Gradient Boosting Classifier Results
![*Figure 7. Confusion Matrix](/home/rstudio/work/results/figures/gbc_confusion_matrix_notuneing.png'){width=500px}

Random Forest Classifier Results
![*Figure 7. Confusion Matrix](/home/rstudio/work/results/figures/rf_confusion_matrix_notuneing.png'){width=500px}


Decision Tree Classifier Results 
![*Figure 7. Confusion Matrix](/home/rstudio/work/results/figures/ada_confusion_matrix_notuneing.png'){width=500px}
